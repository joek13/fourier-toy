const PRESETS = {
  ...
  "butterfly": {
    "x": "2sin(t) * (e^(cos(t)) - 2cos(4t) - sin(t/12)^5)",
    "y": "2cos(t) * (e^(cos(t)) - 2cos(4t) - sin(t/12)^5)"
  },
  "none": {
    "x": "",
    "y": ""
...
function pointToPixel(point) {
  ...
  // positive y faces down in canvas; so we must take its opposite
}

// we use a "circular buffer" here
var points = [];
var index = 0;
var maxPoints = 5000;
...
function addPoint(point) {
  if(points.length < maxPoints) {
    // if we haven't hit max length, grow the array
    points.push(point);
  } else {
    // otherwise, overwrite the oldest written point
